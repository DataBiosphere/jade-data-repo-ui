describe('test query builder on Azure', () => {
  beforeEach(() => {
    cy.server();
    ...
    cy.get('[placeholder="Search keyword or description"]').type('Azure_V2F');
    cy.contains('Date created').click();
    cy.contains(/Azure_V2F_Stats/g).should('be.visible');
    cy.contains(/Azure_V2F_Stats/g).click();
  ...
  describe('test wizard flow', () => {
    it('transitions to share panel', () => {
      ...
      cy.window()
        ...
        .its('snapshotRequest')
        .its('joinStatement')
        .should('match', /FROM (Azure_V2F_Stats).variant /);
    });
  });
});
